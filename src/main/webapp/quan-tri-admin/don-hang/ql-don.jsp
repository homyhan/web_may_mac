...
<%--&lt;%&ndash;											<a&ndash;%&gt;--%>
<%--&lt;%&ndash;													href="/admin/edit-don-hang?idOrder=<%=result.get(i).getInvoice().getIdorder()%>">&ndash;%&gt;--%>
<%--&lt;%&ndash;												<button class="button small green --jb-modal" type="button">&ndash;%&gt;--%>
<%--&lt;%&ndash;													<span class="icon"><span class="mdi mdi-pencil"></span></span>&ndash;%&gt;--%>
<%--&lt;%&ndash;												</button>&ndash;%&gt;--%>
<%--&lt;%&ndash;											</a>&ndash;%&gt;--%>
			...
			<div class="card has-table">
				...
				<div class="card-content">
					<table>
						...
						<tbody>
						<c:if test="${not empty invoiceAdminList}">
							<c:set var="cancelledInvoices" value="${requestScope.cancelledInvoices}" />
							<c:forEach var="invoiceResponse" items="${invoiceAdminList}" varStatus="loop">
								...
								<tr>
									...
									<td data-title="ID"><c:out value="${loop.index + 1}" /></td>
									<td data-title="MADONHANG"><c:out value="${invoiceResponse.getInvoice().getIdinvoice()}" /></td>
									<td data-title="MAORDER"><c:out value="${invoiceResponse.getInvoice().getIdorder()}" /></td>
									<td data-title="TONGGIA">
										<fmt:formatNumber type="number" maxFractionDigits="3" value="${invoiceResponse.getOrder().getGrandtotal()}" /> VND
									</td>
									<td data-title="THANHTOAN"><c:out value="${invoiceResponse.getInvoice().getMode() == Contants.INVOICE_MODE_TRUCTIEP ? 'Thanh toán trực tiếp' : 'Thanh toán qua thẻ'}" /></td>
									<td data-title="NGAYDAT"><c:out value="${invoiceResponse.getInvoice().getCreateAt()}" /></td>
									<td data-title="TRANGTHAI">
										<button class="btn btn-dark" disabled=""><c:out value="${invoiceResponse.getStatusValue()}" /></button>
									</td>
									<!-- Các cột khác -->
									<td data-title="NOTE">
										<c:if test="${cancelledInvoices.contains(invoiceResponse.getInvoice().getIdinvoice())}">
											Đã hủy vì đơn hàng thay đổi
										</c:if>
									</td>
									<!-- Các cột khác -->
									<td class="actions-cell">
										<div class="buttons right nowrap">
											<a href="/admin/chi-tiet-don-hang?idOrder=${invoiceResponse.getInvoice().getIdorder()}">
												<button class="button small green --jb-modal" type="button">
													<span class="icon"><i class="mdi mdi-eye"> </i></span>
												</button>
											</a>
											<form id="select_info_prod" action="/admin/remove-don-hang" method="POST">
												<input name="id" type="hidden" value="${invoiceResponse.getInvoice().getIdinvoice()}" />
												<button class="button small red --jb-modal" data-target="sample-modal" type="submit">
													<span class="icon"><i class="mdi mdi-trash-can" title="Xóa"></i></span>
												</button>
											</form>
											<!-- Các nút khác nếu cần -->
											<form action="/admin/don-hang/updade-status" method="POST">
												<div class="actions-status">
													<input name="idinvoice" type="hidden" value="${invoiceResponse.getInvoice().getIdinvoice()}" />
													<input name="idOrder" type="hidden" value="${invoiceResponse.getOrder().getIdorder()}" />
														<%-- Sử dụng các biến trong phạm vi --%>
													<c:set var="invoiceStatus" value="${invoiceResponse.getInvoice().getStatus()}" />
													<c:set var="isStatusZero" value="${invoiceStatus == 0}" />

													<select class="form-select" name="invoice_status" aria-label="Default select example" ${isStatusZero ? 'disabled' : ''}>
														<option value="0" ${isStatusZero ? 'selected' : ''}>Đã hủy</option>
														<option value="1" ${invoiceStatus == 1 ? 'selected' : ''}>Đã giao</option>
														<option value="2" ${invoiceStatus == 2 ? 'selected' : ''}>Chờ xác nhận</option>
														<option value="3" ${invoiceStatus == 3 ? 'selected' : ''}>Đã xác nhận</option>
														<option value="4" ${invoiceStatus == 4 ? 'selected' : ''}>Đang giao</option>
													</select>

													<button class="button small green --jb-modal" data-target="sample-modal" type="submit">Cập nhật</button>
												</div>
											</form>
						...
						<%--									<td data-title="NOTE">--%>
						<%--										<%--%>
						<%--											Object changedOrderAttr = request.getAttribute("changedOrder");--%>
						<%--											if (changedOrderAttr != null) {--%>
						<%--												int stateNote = (int) changedOrderAttr;--%>
						<%--												if (stateNote == result.get(i).getInvoice().getStatus()) {--%>
						<%--										%>--%>
						<%--										Đã hủy vì dữ liệu đơn hàng bị thay đổi--%>
						<%--										<%--%>
						<%--												}--%>
						<%--											}--%>
						<%--										%>--%>
